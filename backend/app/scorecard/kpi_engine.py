...
class AssessmentKPIInput:
    ...
    decay_weight: float  # pre-computed e^(-λ·age); replaces age_days/period_days

    @property
    def effective_weight(self) -> float:
        return self.assessment_weight * self.dimension_modifier * self.decay_weight
...
def compute_momentum_kpis(
    ...
    """
    current: all assessments with decay weights from today (T=0)
    prior:   same assessments with decay weights from MOMENTUM_WINDOW_DAYS ago (T=-N);
             assessments newer than N days have decay_weight=0 and are filtered out by caller
    """
